<form id="answer-form" method="POST"
        ...
        <div class="choices-container">
            @foreach ($choices as $choice)
                <div class="choice-item">
                    <input type="radio" class="choice-radio" id="choice_{{ $choice->id }}" name="choice_id"
                        value="{{ $choice->id }}"
    ...
    <div class="navigation">
        @if ($quizUser->current_question > 1)
            <button type="submit" name="direction" value="previous" class="nav-button">
        ...
        @if ($quizUser->current_question < count($quizUser->question_order))
            <button type="submit" name="direction" value="next" class="nav-button">
                Next
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"
                    style="width: 20px; height: 20px;">
                    <path fill-rule="evenodd"
